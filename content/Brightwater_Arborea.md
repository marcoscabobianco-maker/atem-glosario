---
title: "Brightwater Arborea"
# === Identidad ===
slug: Brightwater_Arborea
canonical_name: "Brightwater Arborea"
aliases:
 - "Brightwater"
 - "Brightwater (Arborea)"

# === Estrato ontológico (Akashic schema v0.1) ===
stratum: ludic
entity_type: place_narrative
entity_type_iri_hint: "golem:NarrativePlace"

# === Scope ===
campaigns: [atem, mc_core]

# === Relaciones cross-stratum ===
relationships:
 - predicate: located_in
   object: Arborea
   object_stratum: ludic
   notes: "Plano exterior chaotic-good-aligned"
 - predicate: domain_of
   object: Tymora
   object_stratum: ludic
   notes: "Diosa Reinos Olvidados de la fortuna"
 - predicate: birthplace_of
   object: Clarisa
   object_stratum: ludic
   notes: "Nace 1354 DR"
 - predicate: recontextualizes
   object: Brightwater_FR
   object_stratum: ludic
   notes: "Importación canónica de Reinos Olvidados / Faiths & Avatars (TSR 1996)"

# === Capa analítica ===
analytical_notes:
 - by: claude
   date: 2026-05-19
   note: "Importación D&D mantenida sin reescritura. La elección como cuna de Clarisa (bibliotecaria-cronista que pierde su corazón artificial) crea tensión cosmológica: plano de fortuna como cuna de tragedia."
 - by: claude
   date: 2026-05-19
   note: "Pendiente: ¿relación con Arborea_Eden_CDI? El sufijo CDI sugiere parentesco a confirmar."

# === Cockpit Obsidian ===
categoria: lugar
tags:
 - "lugar"
 - "arborea"
 - "plano-exterior"
 - "tymora"
 - "lugar-de-nacimiento"
---

> Reino de la fortuna en el plano de Arborea. Cuna de [[Clarisa]]. Tierra de aventura y de suerte, integrada al cosmos compartido.

## El reino de la fortuna

**Brightwater** —"aguas brillantes"— es el dominio de **Tymora**, la señora de la Suerte, en el plano de **[[Arborea_Eden_CDI|Arborea]]**: tierra de fortuna favorable, hogar de los aventureros que la diosa bendice.

## En el cosmos compartido

Brightwater es **la cuna de [[Clarisa]]**: allí nace, hija del Maestro Bibliotecario del concilio de [[Myth_Drannor]] —[[Arcoms_Cosmar|Arcoms]]— y de [[Nasim]]. Su nacimiento en el plano de la fortuna la distingue de las otras dos que llevan su nombre —la conocida de [[Coyote]], y la madre de [[Elisteo]] con Minaya—; ver [[Clarisa]] para la homonimia.

Que la cuna de una bibliotecaria-cronista —la que escribe el [[Calendario]] y queda atrapada en los hechos cósmicos— sea el plano de la fortuna, es una tensión buscada: la suerte como origen de la tragedia.

## Apariciones

- Nacimiento de [[Clarisa]] en Brightwater.
- [[Clarisa]] — presentación, ascendencia, cronología.

## Ver también

- [[Clarisa]] (nacida en Brightwater)
- [[Arborea_Eden_CDI|Arborea / Eden]] (plano contenedor — confirmar relación)
- [[Arcoms_Cosmar|Arcoms Cosmar]] (padre de Clarisa)
- [[Nasim]] (madre de Clarisa)

---

## Capa interna [R]

> No diegético; no entra a productos públicos.

- **Fuente externa:** *Faiths & Avatars* (TSR, 1996), *Planes of Chaos* (TSR, 1994). Brightwater = dominio de Tymora (Reinos Olvidados) en Arborea; Clarisa nace 1354 DR.
- **Palimpsesto:** importación de Reinos Olvidados sin reinterpretación profunda; la elección de Brightwater como cuna de Clarisa fue producida en situación lúdica.
- **Pendientes:** ¿Brightwater es el mismo lugar que [[Arborea_Eden_CDI|Arborea/Eden]] del Descenso? ¿Tymora tiene presencia explícita en el cosmos compartido? ¿otros compañeros nacen o transitan Brightwater?
