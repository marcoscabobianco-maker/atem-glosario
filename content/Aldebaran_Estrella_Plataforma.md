---
title: "Aldebarán (Estrella / Plataforma)"
categoria: lugar
tags:
 - "paris-ucronica"
 - "cosmología"
 - "plataformas-zodiacales"
aliases:
 - "Aldebarán (Estrella / Plataforma)"
 - "Aldebarán (Estrella"
 - "Plataforma)"
 - "Estrella"
 - "Plataforma"
 - "Aldebarán"
---

## Presentación

Estrella que "guiñó el ojo" a uno de los compañeros pero también "se llevó su sabiduría." Ubicada exactamente a 65.1 años-luz (Alpha Tauri). Mencionada como "plataforma que los persigue o acecha arriba de los cielos vacíos." Contiene un tesoro accesible mediante viaje astral.

**Revelaciones de jornada:**
- El mapa estelar inscrito en la piel de la serpiente voladora muerta señala a Aldebarán como destino de un "tesoro astral"
- Fue la estrella que durante un momento clave "le cortaron la cabeza con Ramón" (referencia a evento de otra ciclo: la fecha donde le cortaron la cabeza a Clona)
- A exactamente 65 [coordenadas]: dato verificable que señala como no inventado

**Revelaciones de jornada:**
- Confirmada a 65.1 años-luz de distancia (dato astronómico real utilizado intencionalmente)
- Origen de [[Zygomind]]: ente gargantuesco que habita en el [[Lago_de_Cali|Lago de Cali]], ubicado en Aldebarán
- La torre del [[Valle_de_la_Muerte_Constantinopla_Anatolia|Valle de la Muerte]] está alineada directamente con Aldebarán (ancla temporal estelar)
- Conjuro Planetario permite visualizar Aldebarán desde espacios cerrados; conjuro InterPlanetario abre portales a su sistema
- Los compañeros usan triangulación estelar con Aldebarán como anclaje para navegación temporal

## Apariciones notables

[[Paris_Ucronica|París Ucrónica]] jornada posterior: múltiples referencias durante exploración y combate. el archivo: identificación de [[Zygomind]] como ente originario de Aldebarán, revelación del [[Lago_de_Cali|Lago de Cali]], alineación de la torre. [[Plataformas_Zodiacales_Guardianes_TNW|Plataformas Zodiacales]] (ver entrada).

## Notas

Aldebarán (Alpha Tauri) es la estrella más brillante de la constelación de Tauro. Su asociación con Tauro conecta con las referencias a "tauro" y al "mes del Sol en Tauro" que el grupo discute en el archivo como fecha significativa (alineación estelar ~21 abril). Ver también **[[Juego_de_las_Sombras|Juego de las Sombras]] / [[Plataformas_Zodiacales_Guardianes_TNW|Plataformas Zodiacales]]**, **[[Serpiente_Voladora_del_Valle|Serpiente Voladora del Valle]]**, **[[Zygomind]]**, **[[Lago_de_Cali|Lago de Cali]]**, **[[Osfiotauro]]**.

## Ver también

- el archivo revelaciones
- el archivo revelaciones

<!-- CICLO -->
> **Casas del ciclo** · ⟳
> *Una estrella que es a la vez puesto de guardia: Aldebarán, el ojo del Toro a 65 años-luz, figura entre las [[Plataformas_Zodiacales_Guardianes_TNW|plataformas zodiacales]] que acechan sobre los cielos vacíos, cada una con su custodio y su tesoro astral. De ella procede [[Zygomind]] en el [[Lago_de_Cali]], y la torre del [[Valle_de_la_Muerte_Constantinopla_Anatolia|Valle de la Muerte]] se alinea con su luz como ancla del que navega el tiempo.* — glosa del archivero del Plata.
<!-- /CICLO -->
