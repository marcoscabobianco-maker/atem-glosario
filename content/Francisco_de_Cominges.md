---
tags: [SaintGermain, MMF, 1648, Francia, personaje, kanka-5406922]
meta: M
image: _assets/SaintGermain/ATEM__SaintGermain__MMF__Francisco_de_Cominges_retrato.png
image_status: pending
nexus_status: APPROVED
---

# ATEM::SaintGermain::MMF::Francisco de Cominges

> [!image-pending] Generar imagen
> **Slot:** `_assets/SaintGermain/ATEM__SaintGermain__MMF__Francisco_de_Cominges_retrato.png`
> **Prompt sugerido:** Ilustración de ATEM::SaintGermain::MMF::Francisco de Cominges. Estilo coherente con la campaña.
> **Notas:** Reemplazar wikilink de abajo cuando este la imagen.

![[ATEM__SaintGermain__MMF__Francisco_de_Cominges_retrato.png]]


**status:** PROPUESTO — requires_user_review
**namespace:** ATEM::SaintGermain::MMF
**kanka_id:** 5406922
**kanka_url:** https://app.kanka.io/w/216093/entities/5406922
**kanka_kind:** character
**kanka_type:** NPC
**familia:** (sin familia)
**sexo:** (no especificado)
**título:** Capitán Teniente General

## Resumen de aparición en transcripts MMF

- **Hits totales** (todas las sesiones): 2
- **Sesiones donde aparece** (2/9):
  - S1 — 1 hits
  - S9 — 1 hits

## Entry Kanka actual

> **(VACÍO en Kanka)** — propuesta: enriquecer desde transcripts MMF.

## Citas y notas de sesión

(Pendientes de extracción detallada — ver matches_por_sesion.json para paragraph_indices exactos.)

## Propuesta ATEM

- **Acción sugerida:** Crear entrada ATEM y proponer enriquecimiento de Kanka con material de transcripts.
- **No mutar Kanka directamente.** Esta entrada queda en staging hasta revisión de Marcos.
- **Cross-ref ATEM:** buscar coincidencias en ATEM/universo/ antes de finalizar (no disponible en este run).
