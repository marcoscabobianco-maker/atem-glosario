---
title: "Clarisa"
# === Identidad ===
slug: Clarisa
canonical_name: "Clarisa"
aliases:
 - "Clarisa"
 - "Clarissa"

# === Estrato ontológico (Akashic schema v0.1) ===
stratum: ludic
entity_type: character_pc
entity_type_iri_hint: "golem:Character"

# === Scope ===
campaigns: [atem, mc_core]

# === Relaciones cross-stratum ===
relationships:
 - predicate: child_of
   object: Arcoms_Cosmar
   object_stratum: ludic
 - predicate: child_of
   object: Nasim
   object_stratum: ludic
 - predicate: located_in
   object: Brightwater_Arborea
   object_stratum: ludic
   notes: "Lugar de nacimiento, 1354 DR"
 - predicate: preceded_by
   object: Amitlai
   object_stratum: ludic
   notes: "En la sucesión de cronistas del Calendario"
 - predicate: inherited_by
   object: Calendario
   object_stratum: ludic
   notes: "Hereda el oficio de cronista — relación inversa: Calendario inherited_by Clarisa"
 - predicate: defeated
   object: Kaxathros
   object_stratum: ludic
   notes: "Negativo: Kaxathros completó la destrucción de Clarisa tras que ella rompiera su corazón artificial (filacteria de Arcoms)"
 - predicate: appears_in_source
   object: C50_p42
   object_stratum: assertoric
 - predicate: appears_in_source
   object: C50_p167
   object_stratum: assertoric
 - predicate: appears_in_source
   object: MC_LINEA_TEMPORAL
   object_stratum: analytical
 - predicate: in_campaign
   object: mc_core
   object_stratum: ludic

# === Capa analítica ===
analytical_notes:
 - by: 
   date: 2026-03-23
   note: "Tres Clarisa distintas: (I) conocida de Coyote 1348 DR; (II) madre de Elisteo con Minaya 1349 DR; (III) hija de Arcoms y Nasim, 1354 DR. Esta entrada es (III)."
 - by: 
   date: 2026-03-23
   note: "Enfrentamiento Clarisa vs Amitlai dentro del juego — ambos compañeros de figura del archivo. Confirmado en el archivo."
 - by: claude
   date: 2026-05-19
   note: "Contradicción aparente: muere 1375 DR (Laveth, ascensión divina) y sale de prisión 1377 DR. Pendiente resolución."
 - by: archivo
   date: 2010-12
   note: "El pez globo: pregunta en archivo oral dic 2010 sobre 'el pez globo... cómo era la historia?' asociada a Clarisa. Respuesta: no estaba escrito, se extraería de cuadernos. Material oral pendiente de minería."

# === Cockpit Obsidian ===
tags:
 - "personaje"
 - "hija-de-arcoms"
 - "bibliotecaria"
 - "calendario"
 - "brightwater"
 - "generación-intermedia"
---

<!-- HEADER_IMG -->
![[Imagenes_Tier1/Clarisa.png]]
<!-- /HEADER_IMG -->

## Presentación

Hija de **[[Arcoms_Cosmar|Arcoms Cosmar]]** y **[[Nasim]]**. Nace en 1354 DR en **[[Brightwater_Arborea|Brightwater Arborea]]**.
Pertenece a la generación intermedia del cosmos compartido (junto con [[Arcoms_Cosmar|Arcoms]], [[Coyote]], [[Minaya_MC|Minaya]],
[[Elisteo]], [[Moran_Atias|Moran Atias]]). Heredera del oficio de su padre: así como [[Arcoms_Cosmar|Arcoms]] fue Maestro
Bibliotecario en el concilio de [[Myth_Drannor]], Clarisa toma la posta como bibliotecaria
y asume la escritura del **[[Calendario]]**, continuando la labor narrativa de **[[Amitlai]]**.

La entrada de Arcoms refiere a "su hija" tres veces sin nombrarla: el amor por ella
lo llevó a unirse a la red de bienhechores; al descubrir cuán traumada estaba, le
entregó sus posibilidades de inmortalidad (filacteria de lich); ella decidió romper
su corazón artificial, y el demonio **[[Kaxathros]]** completó su destrucción.

**Cronología en MC_LINEA_TEMPORAL:**

- **1354 DR:** Nace en [[Brightwater_Arborea|Brightwater Arborea]]. Hija de Arcoms y [[Nasim]].
- **1375 DR:** [[Juicio_Final|Juicio Final (Primer Círculo)]]. **[[Laveth]]** trama su ascensión divina y mata a Clarisa. [Nota: la línea temporal no especifica si es muerte definitiva.]
- **1377 DR:** Clarisa sale de prisión. [Contradicción aparente con la muerte de 1375 — pendiente de resolución: ¿resurrección? ¿muerte no definitiva? ¿homonimia?]

**Homonimia (resuelta):** La línea temporal registra tres "Clarisa" distintas:
(I) conocida de [[Coyote]] en 1348 DR, ref: [814], [1192] — personaje
independiente, no es hija de Arcoms; (II) madre de **[[Elisteo]]** junto con **[[Minaya_MC|Minaya]]**,
1349 DR, plano de los elfos — personaje independiente, no es hija de Arcoms;
(III) hija de Arcoms y [[Nasim]], nacida en 1354 DR — **esta entrada**. La numeración
(I), (II), (III) en la línea temporal debe mantenerse para distinguirlas. Los
eventos de 1375 y 1377 DR corresponden a Clarisa (III).

**Enfrentamiento Clarisa vs. [[Amitlai]]:** Ambos personajes nacieron de la misma
mano. Ambas coinciden cronológicamente en 1375 DR (Juicio
Final, Primer Círculo) y 1377 DR (Falso Secuestro de [[Amitlai]] / Clarisa sale de
prisión). El enfrentamiento entre los dos personajes de la misma mano es un dato
confirmado por el archivo. Circunstancias exactas y otros personajes
presentes: pendiente de rastrear en el archivo.

*Grafía canónica:* **Clarisa** (con una sola S). En el archivo circulan también las variantes
"Clarissa" y "Clarisa"; la forma unificada es **Clarisa**.

## Apariciones notables

El archivo la registra entre 1354 y 1377 DR. Aparece en el esquema de la gesta del rescate
—"Old Father. Arcoms. Daughter. Clarissa. Monster. Abyss. [[Avishai_draconicos_Tiamat|Tiamat]].
Quest. Rescue the Daughter"— y en el diseño de "rescatar a su hija Clarisa o alguna sobrina de
las garras de los Scrupules de Sealing". En las Memorias de [[Amitlai]] no aparece por su nombre,
pero [[Amitlai]] narra su encuentro con Arcoms en [[Islaire]] y el ataque Githyanki a su castillo,
contexto en que Arcoms pierde su torre y su hija queda desprotegida.

## Ver también

- [[Arcoms_Cosmar|Arcoms Cosmar]]
- [[Nasim|Nasim]]
- [[Brightwater_Arborea|Brightwater]]
- [[Calendario|Calendario]]
- [[Amitlai|Amitlai]]
- [[Kaxathros|Kaxathros]]
- [[Cronologia_en_MC_LINEA_TEMPORAL|Cronología en MC_LINEA_TEMPORAL]]
- [[Laveth|Laveth]]
- [[Elisteo|Elisteo]]
- [[Minaya|Minaya]]
